...
public class FuturesTradeLogController {
    ...
    @Operation(summary = "Get recent trade log entries",
               description = "Returns the most recent entries ordered by event timestamp descending. Default limit is 50, max is 500.")
    public ResponseEntity<List<FuturesTradeLog>> getRecent(
            @Parameter(description = "Maximum number of rows to return (1-500, default 50)")
            @RequestParam(defaultValue = "50") int limit) {

        int capped = Math.min(Math.max(limit, 1), 500);
        return ResponseEntity.ok(repository.findRecentLogs(capped));
    }

    @GetMapping("/all")
    @Operation(summary = "Get all trade log entries",
               description = "Returns every row in the trade log ordered by record_timestamp descending. Use /recent for large datasets.")
    public ResponseEntity<List<FuturesTradeLog>> getAll() {
    ...
    @Operation(summary = "Get trade log entries by position ID",
               description = "Returns all lifecycle events recorded for a specific futures position.")
    public ResponseEntity<List<FuturesTradeLog>> getByPositionId(
            @Parameter(description = "The futures position ID") @PathVariable String positionId) {
        return ResponseEntity.ok(repository.findByPositionIdOrderByRecordTimestampDesc(positionId));
    }

    @GetMapping("/order/{orderId}")
    @Operation(summary = "Get trade log entries by order ID",
               description = "Returns all lifecycle events recorded for a specific order.")
    public ResponseEntity<List<FuturesTradeLog>> getByOrderId(
            @Parameter(description = "The order ID") @PathVariable String orderId) {
        return ResponseEntity.ok(repository.findByOrderIdOrderByRecordTimestampDesc(orderId));
    }

    @GetMapping("/pair/{pair}")
    @Operation(summary = "Get trade log entries by trading pair",
               description = "Returns all lifecycle events for a specific futures pair (e.g. BTCUSDT).")
    public ResponseEntity<List<FuturesTradeLog>> getByPair(
            @Parameter(description = "The trading pair symbol") @PathVariable String pair) {
        return ResponseEntity.ok(repository.findByPairOrderByRecordTimestampDesc(pair));
    }
    ...
    public ResponseEntity<List<FuturesTradeLog>> getByEventType(
            @Parameter(description = "The event type") @PathVariable String eventType) {
        return ResponseEntity.ok(repository.findByEventTypeOrderByRecordTimestampDesc(eventType));
    }
    ...
    public ResponseEntity<FuturesTradeLog> getById(
